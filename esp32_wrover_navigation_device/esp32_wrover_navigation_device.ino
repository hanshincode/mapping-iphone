...
int navStatus = 0;            // 0: normal, 1: near turn, 2: rerouting, 3: arrived
...
class MyCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) {
      ...
      if (rxValue.length() > 0) {
        ...
        int fifthSemi = msg.indexOf(';', fourthSemi + 1);
        ...
        if (firstSemi != -1 && secondSemi != -1) {
          ...
          if (thirdSemi != -1 && fourthSemi != -1 && fifthSemi != -1) {
            currentStreet = msg.substring(secondSemi + 1, thirdSemi);
            progressPercent = msg.substring(thirdSemi + 1, fourthSemi).toInt();
            navStatus = msg.substring(fourthSemi + 1, fifthSemi).toInt();
            nextHint = msg.substring(fifthSemi + 1);
          } else {
...
uint16_t statusColor() {
  if (navStatus == 2) return RGB565(255, 60, 60);
  if (navStatus == 3) return RGB565(0, 230, 120);
  if (navStatus == 1) return RGB565(255, 210, 0);
  return RGB565(0, 180, 255);
}

void drawProgressRing() {
  uint16_t trackColor = RGB565(25, 38, 58);
  uint16_t activeColor = statusColor();
  tft.drawCircle(120, 120, 117, trackColor);
  tft.drawCircle(120, 120, 116, trackColor);
  tft.drawCircle(120, 120, 115, trackColor);

  int endDeg = map(progressPercent, 0, 100, 0, 360);
  for (int deg = 0; deg <= endDeg; deg += 3) {
    float rad = (deg - 90) * PI / 180.0;
    int x = 120 + cos(rad) * 116;
    int y = 120 + sin(rad) * 116;
    tft.fillCircle(x, y, 2, activeColor);
  }
}
...
void drawNavigationArrow(int type) {
  int cx = 120;
  int cy = 82;
  uint16_t arrowColor = statusColor();
  
  tft.fillRect(38, 38, 164, 95, BLACK);
...
void updateDisplay() {
  ...
  if (deviceConnected) {
    drawCenteredString(currentDistance, 134, 4, WHITE);
  } else {
    drawCenteredString("MO APP", 145, 3, WHITE);
  }
  
  if (deviceConnected) {
    drawCenteredString(currentStreet, 176, 2, statusColor());
    if (nextHint != "-" && nextHint.length() > 0) {
      drawCenteredString("SAU DO: " + nextHint, 202, 1, RGB565(170, 190, 210));
    }
  } else {
